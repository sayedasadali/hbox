...
openWindowEx title position size (RedrawMode useDoubleBuffer) = do
  ...
  let motionCallback (GL.Position x y) =
        writeTChanIO eventsChan MouseMove { pt = (fromIntegral x, fromIntegral y) }
  GLFW.mousePosCallback $= motionCallback
      
  GLFW.charCallback $= (\char state -> do
    writeTChanIO eventsChan (Key {
        char = char,
        isDown = (state == GLFW.Press) }))

  GLFW.mouseButtonCallback $= (\but state -> do
    GL.Position x y <- GL.get GLFW.mousePos
    writeTChanIO eventsChan (Button {
        pt = (fromIntegral x, fromIntegral y),
        isLeft = (but == GLFW.ButtonLeft),
        isDown = (state == GLFW.Press) }))

  GLFW.windowSizeCallback $= writeTChanIO eventsChan . Resize
  GLFW.windowRefreshCallback $= writeTChanIO eventsChan Refresh
...
closeWindow_ chan = do
  writeTChanIO chan Closed
  modifyMVar_ opened (\_ -> return False)
  GLFW.closeWindow
  GLFW.pollEvents

writeTChanIO :: TChan a -> a -> IO ()
writeTChanIO chan x = atomically $ writeTChan chan x

readTChanIO :: TChan a -> IO a 
readTChanIO chan = atomically $ readTChan chan

isEmptyTChanIO :: TChan a -> IO Bool 
isEmptyTChanIO chan = atomically $ isEmptyTChan chan
...
maybeGetWindowEvent win = do
  updateWindowIfDirty win
  noEvents <- isEmptyTChanIO (eventsChan win)
  if noEvents 
    then GLFW.sleep 0.01 >> GLFW.pollEvents >> return Nothing
    else do
      event <- readTChanIO (eventsChan win)
      case event of
        Refresh -> do
	  ...
	  writeTChanIO (eventsChan win) Refresh
          maybeGetWindowEvent win
        e -> return (Just e)
